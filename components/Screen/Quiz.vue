<template>
  <div class="screen-quiz">
    <div class="screen-quiz--inner">

      <button class="screen-quiz--back" v-if="step !== 1 && step !== 'not'" @click="answer(false, 1)">
      ...
      <svg @click="$parent.quiz = false" class="screen-quiz--close" width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><g clip-path="url(#clip0_31_2381)"><path d="M28.6663 19.333L19.333 28.6663M19.333 19.333L28.6664 28.6663" stroke="#0E0E11" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></g><rect x="1" y="1" width="46" height="46" rx="23" stroke="#0E0E11" stroke-width="2"/><defs><clipPath id="clip0_31_2381"><rect width="16" height="16" fill="white" transform="translate(16 16)"/></clipPath></defs></svg>

      <div class="screen-quiz--item" v-if="step === 'form'">
        <h3>Enter info below to get your results</h3>
        <form class="screen-quiz--form" @submit.prevent="submit">
          <div class="field">
      ...
      <div class="screen-quiz--item" v-else-if="step === 'not'">
      ...
      <div class="screen-quiz--item" v-else>
        <h3>{{ items[step].question }}</h3>

        <div class="screen-quiz--options screen-quiz--number" v-if="items[step].options === 'number'">
        ...
        <div class="screen-quiz--options" v-else>
...
export default {
  data(){
    return {
      ...
      items: {
        ...
        2: {
          question: 'How Many W2 EmployeesDo You Have?', // then this.step= 3
          options: 'number',
          id: 'data3',
          answer: 2
        },
  ...
  methods: {
    ...
    submit(){
      ...
      data.lead_source = localStorage.lead_source ? localStorage.lead_source : ''
      data.utm_source = localStorage.utm_source ? localStorage.utm_source : ''
      data.utm_medium = localStorage.utm_medium ? localStorage.utm_medium : ''
      data.utm_campaign = localStorage.utm_campaign ? localStorage.utm_campaign : ''
      data.c1 = localStorage.c1 ? localStorage.c1 : ''
      data.c2 = localStorage.c2 ? localStorage.c2 : ''
      data.c3 = localStorage.c3 ? localStorage.c3 : ''
      data.c4 = localStorage.c4 ? localStorage.c4 : ''
      data.ssid = this.$refs.leadid_token.value
      data.ef_aff = localStorage.c3 ? localStorage.c3 : ''
      data.ef_sub1 = localStorage.sub1 ? localStorage.sub1 : ''
      data.ef_sub2 = localStorage.sub2 ? localStorage.sub2 : ''
      data.ef_sub3 = localStorage.sub3 ? localStorage.sub3 : ''
      data.ef_sub4 = localStorage.sub4 ? localStorage.sub4 : ''
      data.ef_trans = localStorage.c1 ? localStorage.c1 : ''
      data.source = 'everflow'
      data.optinurl = localStorage.optinurl ? localStorage.optinurl : ''
      data.is_owner = this.items[6].answer
      ...
      this.$axios.get(this.setUrl(data))
        ...
        .then(res => {
          console.dir(res)
          this.$parent.quiz = false
          this.$parent.sent = true
        })
        .catch(err => {
          console.dir(err)
          this.$swal(err)
